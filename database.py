...
if __name__ == "__main__":
    ...
    try:
        # Initialize database
        db = AdvancedDatabase("./test_rayonix_db", config)
        
        # Create indexes for transactions
        tx_index_config = IndexConfig(
            index_type=IndexType.BTREE,
            fields=['sender', 'receiver', 'block_height'],
            unique=False
        )
        db.create_index("transactions", tx_index_config)
        
        # Create compound index for sender + timestamp
        compound_config = IndexConfig(
            index_type=IndexType.COMPOUND,
            fields=['sender', 'timestamp'],
            unique=False
        )
        db.create_index("sender_timestamp", compound_config)
        
        # Store sample transactions
        transactions = [
            Transaction(
                hash=f"tx_{i}",
                sender=f"address_{i % 10}",
                receiver=f"address_{(i + 5) % 10}",
                amount=100.0 + i,
        ...
        for tx in transactions:
            key = f"tx:{tx.hash}".encode()
            value = asdict(tx)
            batch_ops.append(BatchOperation('put', key, value))
        
        success = db.batch_write(batch_ops)
        print(f"Batch insert: {'Success' if success else 'Failed'}")
        
        # Query using secondary index
        print("\nQuerying transactions for address_5:")
        results = db.query("transactions", "address_5", limit=5)
        for result in results:
            print(f"  TX: {result['hash']} - Amount: {result['amount']}")
        
        # Complex query using query builder
        print("\nComplex query - transactions from address_3 with amount > 105:")
        query = QueryBuilder(db).where('sender', '==', 'address_3').where('amount', '>', 105).limit(5)
        ...
        try:
            with transaction(db) as batch:
                ...
                batch.append(BatchOperation(
                    ...
                    b"tx:tx_test_001", 
                    asdict(new_tx),
                    index_updates=db._calculate_index_updates(
                        b"tx:tx_test_001", asdict(new_tx), None, None
                    )
                ))
                
                # This will commit automatically if no exceptions
        ...
        stats = db.get_stats()
        print(f"\nDatabase statistics:")
        for key, value in list(stats.items())[:10]:  # Show first 10 stats
            print(f"  {key}: {value}")
        
        # Test health monitor
        monitor = DatabaseHealthMonitor(db)
        monitor.start()
        time.sleep(2)  # Let it collect some metrics
        monitor.stop()
        
        print(f"\nHealth metrics collected: {len(monitor.get_metrics(60))}")
