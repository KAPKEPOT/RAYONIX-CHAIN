...
if __name__ == "__main__":
    ...
    data = ["tx1", "tx2", "tx3", "tx4", "tx5", "tx6", "tx7", "tx8"]
    ...
    # Test proof generation and verification in binary format
    proof_data = tree.get_proof("tx2", ProofFormat.BINARY)
    if proof_data:
        print(f"Binary proof size: {len(proof_data)} bytes")
        
        is_valid = tree.verify_proof(proof_data, tree._hash_data("tx2"), tree.get_root_hash())
        print(f"Proof valid: {is_valid}")
    
    # Test JSON proof format
    json_proof = tree.get_proof("tx3", ProofFormat.JSON)
    if json_proof:
        print(f"JSON proof size: {len(json_proof)} bytes")
    
    # Test async tree building
    async def test_async_build():
        async_tree = MerkleTree([], config)
        await async_tree.build_tree_async(data)
        print(f"Async root hash: {async_tree.get_root_hash()}")
    
    asyncio.run(test_async_build())
    
    # Test factory pattern
    factory = MerkleTreeFactory(config)
    factory.create_tree("test_tree", data)
    retrieved_tree = factory.get_tree("test_tree")
    print(f"Factory tree root: {retrieved_tree.get_root_hash() if retrieved_tree else 'None'}")